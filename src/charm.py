...
class PortainerCharm(CharmBase):
    ...
    def _update_pebble(self, event, config: dict):
        ...
        if container.can_connect():
            ...
            container.add_layer(CONTAINER_NAME, self._build_layer_by_config(config), combine=True)
    ...
    def _on_config_changed(self, event):
        ...
        if new_config[CONFIG_SERVICETYPE] != self._config[CONFIG_SERVICETYPE] and (
            new_config[CONFIG_SERVICETYPE] == SERVICETYPE_NP
    ...
    def _start_portainer(self, _):
        ...
        if container.can_connect():
            ...
            if not svc:
                # Add a new layer
                container.add_layer(
                    CONTAINER_NAME, self._build_layer_by_config(self._config), combine=True
                )
                container.start(CONTAINER_NAME)
    ...
    def _patch_k8s_service_by_config(self, new_config: dict):
        ...
        spec = utils.clean_nones(
            client.sanitize_for_serialization(self._build_k8s_spec_by_config(new_config))
        )
        body = []
    ...
    def _create_k8s_service_by_config(self):
        ...
        api = kubernetes.client.CoreV1Api(kubernetes.client.ApiClient())
        try:
            api.delete_namespaced_service(name=self.app.name, namespace=self.namespace)
        except kubernetes.client.exceptions.ApiException as e:
            if e.status == 404:
    ...
    def _create_k8s_service_account(self) -> bool:
        ...
        try:
            ...
            if e.status == 404:
                logger.error(
                    f"{CLUSTERROLE_NAME} cluster role doesn't exist, please make sure RBAC is enabled in k8s cluster."
                )
                return False
            else:
                raise e
        ...
        try:
            api.delete_namespaced_service_account(
                name=SERVICEACCOUNT_NAME, namespace=self.namespace
            )
        except kubernetes.client.exceptions.ApiException as e:
            if e.status == 404:
    ...
    def _validate_config(self, config: dict) -> bool:
        ...
        if config.get(CONFIG_SERVICETYPE) not in (SERVICETYPE_CIP, SERVICETYPE_LB, SERVICETYPE_NP):
            logger.error(
                f"config - service type {config.get(CONFIG_SERVICETYPE)} is not recognized"
            )
            return False
        if (
            config.get(CONFIG_SERVICEHTTPPORT) is None
            or config.get(CONFIG_SERVICEEDGEPORT) is None
        ):
        ...
        if config.get(CONFIG_SERVICEHTTPPORT) == config.get(CONFIG_SERVICEEDGEPORT):
            ...
            return False
        if (
            config.get(CONFIG_SERVICEHTTPNODEPORT) == config.get(CONFIG_SERVICEEDGENODEPORT)
    ...
    def _build_k8s_spec_by_config(self, config: dict) -> kubernetes.client.V1ServiceSpec:
        ...
        if service_type == SERVICETYPE_NP and CONFIG_SERVICEHTTPNODEPORT in config:
        ...
        if service_type == SERVICETYPE_NP and CONFIG_SERVICEEDGENODEPORT in config:
    ...
    def _build_layer_by_config(self, config: dict) -> dict:
        ...
        cmd = "/portainer"
        if config[CONFIG_SERVICETYPE] == SERVICETYPE_NP and CONFIG_SERVICEEDGENODEPORT in config:
